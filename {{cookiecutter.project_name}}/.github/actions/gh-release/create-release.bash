...
```sh
gh attestation verify {{ cookiecutter.project_name }}-linux-amd64-*.tar.gz --repo @REPO@
```
...
```sh
cosign verify-blob \
  --bundle {{ cookiecutter.project_name }}-*.spdx.json.sigstore.json \
  --certificate-identity-regexp "https://github.com/@REPO@/.github/workflows/release.yml@refs/tags/.*" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" \
  {{ cookiecutter.project_name }}-*.spdx.json
```
EOF
)"
...
gh release create "${TAG}" \
    ...
    "${DIST_DIR}/{{ cookiecutter.project_name }}-${TAG}.spdx.json" \
    "${DIST_DIR}/{{ cookiecutter.project_name }}-${TAG}.spdx.json.sigstore.json"
